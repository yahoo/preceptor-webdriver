...
var WebDriverManager = Base.extend(
...

	{
		...
		hasClient: function (name) {
			return !!this.getClientList()[name];
		},
		...
		getClient: function (name) {
			var client = this.getClientList()[name];

			if (!client) {
				throw new Error("Cannot find client: " + name);
			}
		...
		hasServer: function (name) {
			return !!this.getServerList()[name];
		},
		...
		getServer: function (name) {
			var server = this.getServerList()[name];

			if (!server) {
				throw new Error("Cannot find server: " + name);
			}
